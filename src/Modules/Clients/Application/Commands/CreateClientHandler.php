...
final readonly class CreateClientHandler
{
     ...
     * }  $attributes
     */
    #[\NoDiscard('handle() returns the created client.')]
    public function handle(array $attributes, int $userId): ClientData
    {
        $client = ClientEloquentModel::query()->create([
            'user_id' => $userId,
            'client_name' => $attributes['client_name'],
            'email' => $attributes['email'] ?? null,
            'status' => $attributes['status'] ?? ClientStatus::Draft->value,
