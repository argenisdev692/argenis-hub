 ...
 * Responses are documented by Scramble from the return types. Authorization is
 * route middleware (`permission:*_CVS`) plus per-row ownership.
 *
 * Filters are documented from the injected {@see CvFilterData}: Scramble reads a
 * `Data` parameter's rules directly, while the equivalent
 * `CvFilterData::validateAndCreate($request)` call hides them — the rules live
...
final readonly class CvApiController
{
     ...
     *
     * `search` matches the CV title, its original filename or its niche.
     */
    #[QueryParameter(
        'per_page',
