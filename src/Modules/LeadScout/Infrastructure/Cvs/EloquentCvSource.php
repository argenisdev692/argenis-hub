...
/**
 * Reads the operator's `cvs` rows: own user, not deleted, primary with
 * extracted text by default (md or PDF — `raw_text` is format-agnostic
 * since `CvTextExtractor` parses PDFs). `raw_text` is returned only to the
 * import handler — never to controllers, logs or the IA.
 */
    ...
    public function cvsForUser(int $userId): array
    {
        return CvEloquentModel::query()
            ...
            ->map(static fn (CvEloquentModel $cv): CvOption => new CvOption(
                ...
                importable: $cv->raw_text !== null && trim($cv->raw_text) !== '',
            ))
            ->all();
    }
