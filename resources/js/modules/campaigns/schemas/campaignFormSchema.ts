...
/**
 * The client-side mirror of `UpdateCampaignData::rules()`.
 *
 ...
 * does not accept it. A campaign still mid-generation is not something a human
...
export const campaignFormSchema = z
    .object({
        ...
        call_to_action: z
            ...
            .max(100, 'Call to action must be 100 characters or fewer.'),
        hashtags: z
            .array(
                z
                    .string()
        ...
        lead_form_questions: z
            .array(
                z
                    .string()
                    .trim()
                    .max(255, 'Each question is 255 characters or fewer.'),
            )
            .max(10, 'Up to 10 lead form questions.'),
        status: z.enum(CAMPAIGN_EDITABLE_STATUSES),
...
/** The body `PUT /campaigns/{uuid}` accepts — mirrors `UpdateCampaignData`. */
export type CampaignWritePayload = {
    ...
    status: (typeof CAMPAIGN_EDITABLE_STATUSES)[number];
...
/**
 * Seeds the review form from a stored campaign.
 *
 * A campaign that has never finished generating has null copy fields; they
 * become empty strings so the form's own `required` messages fire instead of
 * Vue warning about a null `value` binding.
 */
export function toCampaignFormValues(
    campaign: CampaignDetail,
): CampaignFormValues {
    const status = CAMPAIGN_EDITABLE_STATUSES.find(
        (value) => value === campaign.status,
    );
    ...
    return {
        ...
        lead_form_questions: campaign.lead_form_questions ?? [],
        // `generating` has no editable counterpart — fall back to the state a
        // human would move it to next.
        status: status ?? 'draft',
        scheduled_at: toLocalDateTimeInput(campaign.scheduled_at),
    };
}
...
export function toWritePayload(
    ...
    return {
        ...
        primary_text: values.primary_text.trim(),
        // `nullable` on the server, and an empty textarea is "no description",
        ...
        hashtags: values.hashtags
            .map((tag) => tag.trim())
            .filter((tag) => tag !== ''),
        lead_form_questions: values.lead_form_questions
