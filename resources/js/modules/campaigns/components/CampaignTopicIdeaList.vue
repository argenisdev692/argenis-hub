...
import type { CampaignTopicIdea } from '../types';

/**
 * The ideation results — presentational only. Picking one does not spend a
 * generation call; it fills the brief, and the page decides what to do with it.
 * Keeping that split is what lets a user audition ten angles for one
 ...
 * DTO types it as a string — unlike `estimated_virality`, `estimated_roi` and
 * `estimated_lead_potential`, which are 0–100 numbers and get the shared score
 * tone.
 *
        ...
        >
            ...
            <p class="mt-2 flex flex-wrap gap-2 text-xs text-muted-foreground">
                ...
                <Badge variant="outline">
                    {{ campaignFunnelStageLabel(idea.funnel_stage) }}
                </Badge>
