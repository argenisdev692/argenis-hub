...
import {
    ...
    scoreTextClass,
    scoreTone,
} from '../helpers/campaignPresentation';

/**
 * The five quality signals the generation loop optimises against, plus the AI
 * detection risk the evaluator reports alongside them, as labelled meters.
 *
 * These are the numbers the loop actually iterates on:
 * `CampaignQualityEvaluator` re-runs generation until all five clear their
 * thresholds or the iteration budget is spent. Showing them together is the
 * point — an 88 virality beside a 54 lead quality is a different media-buying
 * decision than either number alone.
 *
...
const {
    audienceFitScore = null,
    viralityScore = null,
    roiPotentialScore = null,
    leadQualityScore = null,
    trendRelevanceScore = null,
    aiDetectionRisk = null,
    compact = false,
} = defineProps<{
    audienceFitScore?: number | null;
    viralityScore?: number | null;
    roiPotentialScore?: number | null;
    leadQualityScore?: number | null;
    trendRelevanceScore?: number | null;
