...
> **Binary**: PHP 8.5 provided by **Laravel Herd (native Windows)** — no Docker, no Sail, no WSL. Validate ALL code against this runtime. CLI: `php artisan`, `composer`, `vendor/bin/pint`, `php artisan test`.
> **Stack**: PHP 8.5 · Laravel 13 · Spatie Permission 8.x · Spatie Laravel Data 4.x · Pest 5.x + PHPUnit 13.x
...
| --- | --- | --- | --- |
| **Pipe operator `\|>`** | PHP 8.5 | `Application/Commands/` handlers, `Infrastructure/Http/Export/*Transformer` (`forExcel`/`forPdf`), `Application/DTOs/` Data mappers, entity normalization methods | ✅ Yes |
...
**MUST use for**: `Application/Commands/`, `Application/DTOs/` (Spatie Data), `Infrastructure/Persistence/Export/`, sanitization pipelines, any sequential transformation.
...
| `app/Providers/EventServiceProvider.php` event maps | `Event::listen(...)` / `Event::subscribe(...)` in the module ServiceProvider, or `->withEvents(discover: [...])` in `bootstrap/app.php` |
...
| `./vendor/bin/sail artisan` (Docker/WSL) | `php artisan` directly — Herd native (project rule) |
...
| Events | ❌ **no attribute exists** — verified against Laravel v13.26 (2026-08). Use `Event::listen()` in the module ServiceProvider | `EventServiceProvider::$listen` | — (`Illuminate\Events\Attributes` does NOT exist) |
...
**`getActivitylogOptions()` rules**: `logOnly([...])` explicit, never `logAll()`. Never log passwords/tokens. `logOnlyDirty()` + `dontLogEmptyChanges()` mandatory.
...
| `laravel/ai` ^0.11            | `Shared/Infrastructure/AI/LaravelAIAdapter.php` — the official Laravel AI SDK. Use this for ALL LLM / embedding calls in this project. Wrap every adapter behind `AIClientInterface` so OWASP §16 (PII redaction, rate limit, timeout, audit) applies uniformly. Do NOT install `prism-php/prism` or any third-party LLM bridge alongside. |
...
| 404 after route changes                     | `php artisan optimize:clear` (or `php artisan config:clear && php artisan cache:clear && php artisan route:clear && php artisan view:clear`) |
