...
<!-- Canonical source — edits go HERE; run npm run syn to re-sync .cursor/ and .claude/. -->
...
> Full canonical rule lives in `.opencode/skills/BACKEND-PHP/SKILL.md` §5. Replicated here so this skill is self-contained when loaded in isolation.
...
### Cross-module boundaries — a module's public API

Code outside a module may ONLY import from another module's **public API**:

| Allowed from another module                          | Meaning |
|---|---|
| `Modules\{X}\Domain\Events\*`                        | published facts (subscribe via `#[AsEventListener]`) |
| `Modules\{X}\Application\DTOs\*`                     | shared read/write shapes |
| `Modules\{X}\Domain\Ports\*`                         | contracts the module exposes |
| `Modules\{X}\Providers\*`                            | wiring only (`bootstrap/providers.php`) |

**NEVER import another module's `Infrastructure\*`, `Application\Commands\*`, `Application\Queries\*`, or `Application\Listeners\*`.** A module's Infrastructure is private. Cross-module reads go through the owning module's Ports or a query contract; cross-module reactions go through its Domain Events. Break cycles with events or a Shared contract — never with a direct import.

**Exemptions (composition roots):** `Database\Seeders`, `Database\Factories`, and `Tests` may import anything.

**Mechanical enforcement:** `tests/Architecture/LayersTest.php` (Pest `arch()`) enforces the layer table and the Infrastructure privacy rule. Extend it when adding new boundary rules — a rule that is not tested is a rule that will rot. Run: `php artisan test --compact tests/Architecture`.
...
> **For architecture rules** (date handling, property naming, cache management, readonly classes) → see `.opencode/skills/BACKEND-PHP/SKILL.md` §5.
...
- **Scramble auto-generation rules** (return type-hints, FormRequest injection, Spatie Data responses) → `.opencode/skills/BACKEND-PHP/SKILL.md` §9.
- **API routes convention** (file location, ServiceProvider registration, route order, permission middleware) → `.opencode/skills/BACKEND-PHP/SKILL.md` §6 (Routes Convention).
