...
| `php artisan` directly | `php artisan` (project rule) |
...
> **This table is a backend quick-reference, NOT the full security baseline.** The complete control set — including the §15 SSRF carve-out (folded into A01:2025) and §16 OWASP Top 10 for LLM Applications:2025 (mandatory whenever `Shared/Infrastructure/AI/` is active) — lives in `.opencode/skills/OWASP/SKILL.md`. Read both files before any backend change.
...
| 404 after route changes                     | `php artisan config:clear && php artisan cache:clear && php artisan route:clear && php artisan view:clear` |
