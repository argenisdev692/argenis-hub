...
## §4.5 — Vue 3.5 Core APIs (MANDATORY)

> **Authority**: the floor is Vue 3.5 stable (§0 stack line). These APIs are **not optional modernisation** — they are the current idiom, and the pre-3.5 equivalents are FAIL in new code. Vue 3.6 Vapor mode stays banned until GA.

### `useTemplateRef()` — template refs (replaces the matching-name `ref`)

```ts
import { useTemplateRef } from 'vue'

const dialogRef = useTemplateRef<InstanceType<typeof Dialog>>('confirm-dialog')
...
<Dialog ref="confirm-dialog" />
```

- The string argument is the `ref` attribute value; it does **not** have to match the variable name.
- Always pass the element/component generic — an untyped `useTemplateRef()` is `unknown` and fails strict mode.
- ❌ BANNED: `const el = ref<HTMLElement | null>(null)` paired with `ref="el"`. ESLint `vue/prefer-use-template-ref` enforces this.

### `useId()` — SSR-stable unique IDs (accessibility, §2)
...
<input :id="fieldId" :aria-describedby="errorId" :aria-invalid="Boolean(error)" />
<small :id="errorId">{{ error }}</small>
```

- **MANDATORY** for every `label ↔ control` pair and every `aria-describedby` / `aria-labelledby` / `aria-controls` link in a custom component.
- IDs are stable across server and client render, so SSR (§5) hydrates without mismatch.
- ❌ BANNED: `Math.random()`, `crypto.randomUUID()`, incrementing module counters, or hardcoded `id` strings in reusable components (they collide when the component is rendered twice on one page).

### Reactive props destructuring (stable in 3.5 — replaces `withDefaults`)

```ts
// ✅ Vue 3.5 — destructured props stay reactive, defaults are plain JS
const { severity = 'secondary', size = 'normal', disabled = false } = defineProps<{
...
- ❌ BANNED: `withDefaults(defineProps<T>(), { ... })` in new code.
- Passing a destructured prop into a composable, `watch`, or `computed` that expects a source requires a **getter**, because destructuring yields a value, not a ref:

```ts
watch(() => severity, (next) => { /* ... */ })     // ✅
useSomething(() => severity)                        // ✅ MaybeRefOrGetter
watch(severity, ...)                                // ❌ not reactive
```

- When the whole props object is needed (spreading, `v-bind="props"`), keep `const props = defineProps<T>()`. Both forms are valid; do not mix them in one component.
- ESLint `vue/define-props-destructuring` enforces the chosen style.

### `defineModel()` — two-way bindings (replaces manual `modelValue` + `update:modelValue`)

```ts
const search = defineModel<string>({ default: '' })
const open = defineModel<boolean>('open', { required: true })
const range = defineModel<[Date, Date] | null>('range')
```
...
- **MANDATORY** for every custom form control (`DataTableDateRangeFilter.vue`, status select, any Volt wrapper exposing `v-model`).
- ❌ BANNED: declaring `modelValue` in `defineProps` and emitting `update:modelValue` by hand.

### `onWatcherCleanup()` — cancel in-flight side effects

```ts
import { watch, onWatcherCleanup } from 'vue'

watch(() => toValue(filters).search, (term) => {
    const controller = new AbortController()
    const timer = setTimeout(() => void doSearch(term, controller.signal), 300)
...
- **MANDATORY** wherever a watcher starts a timer, subscription, or `fetch`: the debounced search (§7.1), export polling (§12.1), and the theme watcher (§1.5).
- Must be called **synchronously** during the watcher callback — not after an `await`.
- Prefer `watch(..., { once: true })` over a manual `stop()` when the watcher should fire exactly once.

### `defineSlots<T>()` — typed slots

```ts
defineSlots<{
    default(props: { row: UserListItem; index: number }): unknown
    empty?(): unknown
    footer?(props: { total: number }): unknown
}>()
```

- **MANDATORY** on any `common/` or `modules/` component exposing a scoped slot. Untyped scoped slots are `any` at the call site and violate the no-`any` rule (§13).

### Lazy hydration (SSR only)

```ts
import { defineAsyncComponent, hydrateOnVisible, hydrateOnIdle } from 'vue'

const ActivityChart = defineAsyncComponent({
    loader: () => import('@/modules/dashboard/components/ActivityChart.vue'),
    hydrate: hydrateOnVisible(),
})
```

- Use `hydrateOnVisible()` for below-the-fold widgets (charts, activity feeds) and `hydrateOnIdle()` for non-critical panels — only when Inertia SSR is enabled (§5).
- Never lazy-hydrate an interactive control that is above the fold.

### Hard rules — Vue 3.5

1. Template refs → `useTemplateRef<T>('name')`. Never a same-name `ref()`.
2. Every generated `id` in a reusable component → `useId()`. Never random or hardcoded.
3. `withDefaults` is banned; use destructured defaults, and a getter when passing a destructured prop as a reactive source.
4. Every custom `v-model` → `defineModel<T>()`.
5. Every watcher that starts a timer / request / subscription → `onWatcherCleanup()`.
6. Every scoped slot → `defineSlots<T>()`.
7. Vapor mode (`3.6.x`) remains BANNED in production until GA.
...
**Returned reactive refs**: `data`, `state`, `status`, `asyncStatus`, `isPending`, `isLoading`, `isPlaceholderData`, `error`, `refetch`, `refresh`.

- `isPending` — `status === 'pending'`, i.e. the **first** load only.
- `isLoading` — `asyncStatus === 'loading'`, i.e. **any** fetch in flight (including background refetches).
- `isPlaceholderData` — `data` is still the previous page. Use it to dim the DataTable while the next page loads.

### Reusable Query / Mutation (`defineQuery` / `defineMutation`)

A plain composable creates **fresh local state per caller** — two components calling `use{Entities}()` get two independent `filters` refs and de-synchronise. When the state around the query must be shared (a filter bar in the toolbar driving a table in the page body), wrap it with `defineQuery`, which instantiates once and shares:
...
export const use{Entities} = defineQuery(() => {
    // `filters` is shared by EVERY component that calls use{Entities}()
    const filters = ref<{Entity}Filters>({ search: '', status: 'all', page: 1 })

    const { state, ...rest } = useQuery<PaginatedResponse<{Entity}ListItem>>({
        key: () => ['{entities}', filters.value],
        query: () => fetch{Entities}(filters.value),
        staleTime: 1000 * 60 * 2,
        gcTime: 1000 * 60 * 5,
        placeholderData: (previousData) => previousData,
    })

    return { ...rest, {entities}: state, filters }
})
```

`defineMutation` does the same for writes that carry their own form state:

```ts
export const useCreate{Entity} = defineMutation(() => {
    const draft = ref<Create{Entity}Payload>(empty{Entity}())
    const { mutate, ...mutation } = useMutation({ /* ... */ })

    return { ...mutation, draft, submit: () => mutate(draft.value) }
})
```

**Decision rule** — `defineQuery`/`defineMutation` when the surrounding state (filters, draft, selection) is shared across components; the plain `use{Entities}(filters)` parameterised form when each caller owns its own filters. Never solve sharing by lifting the state into a Pinia store: that duplicates server state and violates the state-ownership rule (§0).
...
export function use{Entity}Mutations() {
    ...
    const delete{Entity} = useMutation({
        ...
        // Optimistic delete
        onMutate(uuid) {
            type List = PaginatedResponse<{Entity}ListItem> | undefined

            const previous = queryCache.getQueryData<List>(['{entities}'])
            const optimistic: List = previous
                ? { ...previous, data: previous.data.filter(r => r.uuid !== uuid) }
                : previous

            queryCache.setQueryData(['{entities}'], optimistic)
            // Cancel in-flight fetches so a stale response cannot overwrite the optimistic value.
            queryCache.cancelQueries({ key: ['{entities}'] })

            // Return BOTH so onError can verify before rolling back.
            return { previous, optimistic }
        },
        onError(err: Error, _uuid, context) {
            // Roll back ONLY if the cache still holds OUR optimistic value — another
            // mutation or refetch may have landed in between, and blindly restoring
            // the snapshot would discard it.
            if (context?.optimistic === queryCache.getQueryData(['{entities}'])) {
                queryCache.setQueryData(['{entities}'], context.previous)
            }
...
3. **Cache invalidation**: `queryCache.invalidateQueries({ key: ['{entities}'] })` after every successful write, from `onSettled` (runs on both success and error).
4. **Cache lifetime**: every query declares BOTH `staleTime` (when to refetch — default 2 min for lists) and `gcTime` (when to evict — default 5 min). A query with neither is a FAIL.
5. **Pagination**: `placeholderData: (previousData) => previousData` on every paginated list query. ❌ BANNED: manually holding the last page in a local `ref` to avoid an empty-table flash — that is what `placeholderData` is for.
6. **Optimistic updates** — all four steps, in this order:
   1. `getQueryData` → snapshot `previous`;
   2. `setQueryData` → write the optimistic value;
   3. `cancelQueries` → stop in-flight fetches overwriting it;
   4. return **both** `previous` and `optimistic` from `onMutate`.
   In `onError`, roll back **only if** `context.optimistic === queryCache.getQueryData(key)` — an unguarded rollback discards a concurrent mutation. In `onSuccess`, reconcile the optimistic row with the server payload (real `uuid`, `created_at`, computed columns) so rapid successive mutations converge.
7. **Errors**: surface user-friendly messages via PrimeVue `useToast()` (`toast.add({ severity: 'error', ... })`). Never log raw `4xx`/`5xx` bodies in the UI.
8. **Loading state**: `isPending` = first load; `isLoading` = any fetch in flight; `isPlaceholderData` = showing the previous page. Feed `isPending` into the DataTable `:loading` prop (§7) and use `isPlaceholderData` to dim rows during page changes.
9. **Sharing**: when filters/draft state is shared across components, use `defineQuery` / `defineMutation`. Never lift server state into a Pinia store.
10. **Decoupling**: composables live in `modules/{context}/composables/`. Pages consume them; pages never call `fetch` directly.
...
> **ABSOLUTE — zero `any`**: no `any`, no `as any`, no implicit `any`, no `@ts-ignore`, no bare `@ts-expect-error`. When a value is genuinely unknown, type it `unknown` and narrow with a type guard.

### Backend types are GENERATED — never hand-written

`spatie/laravel-typescript-transformer` v3 reads the Spatie `Data` classes and enums and emits the TypeScript. **Hand-writing an interface that mirrors a backend `Data` class is a FAIL** — it is how `any` and silent drift get in.

```bash
php artisan typescript:transform            # → resources/js/generated/generated.d.ts
php artisan typescript:transform --watch    # during development
```

Run it **the first time a frontend module is created** and again after **any** backend DTO/enum change. It is part of the backend module finalization pipeline (`rules.md`), so a green backend module always ships current frontend types.

**What you get, for free:**

- Every `Spatie\LaravelData\Data` subclass — no `#[TypeScript]` attribute required (`DataClassTransformer::shouldTransform()` matches any `BaseData` implementor). Backed enums are picked up by `EnumTransformer` with no attribute either.
- `#[MapOutputName(SnakeCaseMapper::class)]` and `#[MapName]` are **honored by the generator**, so the snake_case contract below is produced automatically — not maintained by hand.
- `Lazy` / `Optional` properties are unwrapped (`RemoveLazyTypeProcessor`, `RemoveOptionalTypeProcessor`) — `Inertia::optional()` props land as optional TS members.
- `PaginatedDataCollection<TKey, TValue>` and `CursorPaginatedDataCollection<...>` are emitted as aliases over `LengthAwarePaginator` / `CursorPaginator` — **use these instead of a hand-rolled `PaginatedResponse<T>`**.
- Enums via `EnumTransformer` — every backed enum in the scanned directories, no attribute needed.

**Two setup constraints (both verified the hard way):** the output directory must exist before the command runs (it is not created), and `resources/js/generated/` must NOT be gitignored — `PrettierFormatter` shells out to `npx prettier`, which in Prettier 3 reads `.gitignore` by default and would silently skip the file.

Consume them as ambient globals (the writer is `GlobalNamespaceWriter`), or import from the generated file:

```ts
// resources/js/modules/{context}/composables/use{Entities}.ts
import type { App } from '@/generated/generated'

type {Entity}ListItem = App.Modules.{Module}.Application.DTOs.{Entity}Data
```

### What you still hand-write

Only shapes with **no backend counterpart** — client-only UI state, and the Inertia page-props augmentation that wires the generated types in:
...
declare module '@inertiajs/vue3' {
    interface PageProps {
        auth: { user: App.Data.AuthUserData }        // ← generated, not hand-written
        flash?: { success?: string; error?: string } // ← no backend Data class, hand-written OK
    }
}
```
...
1. `php artisan typescript:transform` is the **only** source of backend-shaped TypeScript. Regenerate; never edit `generated.d.ts`.
2. A hand-written interface duplicating a `Data` class → **FAIL**. Delete it and import the generated type.
3. `generated.d.ts` is a build artifact: it is regenerated by the pipeline, so never patch it to "fix" a type — fix the PHP `Data` class instead.
4. Zero `any` (see the ABSOLUTE note above). `unknown` + narrowing is the escape hatch.
5. Client-only shapes (UI state, form drafts with no DTO, navigation config) stay hand-written under `types/`.
...
- [ ] `queryCache.invalidateQueries({ key: ['{entities}'] })` from `onSettled` after every write
- [ ] Every query declares BOTH `staleTime` AND `gcTime`
- [ ] Every paginated list query uses `placeholderData: (previousData) => previousData` — no manual "last page" ref
- [ ] Optimistic mutations: `onMutate` returns BOTH `previous` and `optimistic`; `onError` rolls back ONLY when `context.optimistic === queryCache.getQueryData(key)`; `onSuccess` reconciles with the server payload
- [ ] Shared filter/draft state uses `defineQuery` / `defineMutation` — never a Pinia store holding server state
- [ ] **Vue 3.5 (§4.5)**: `useTemplateRef<T>('name')` for every template ref (no same-name `ref()`)
- [ ] **Vue 3.5 (§4.5)**: `useId()` for every generated `id` / `aria-*` link in a reusable component
- [ ] **Vue 3.5 (§4.5)**: destructured props with defaults — `withDefaults` is banned; getters (`() => prop`) when passing a destructured prop as a reactive source
- [ ] **Vue 3.5 (§4.5)**: `defineModel<T>()` for every custom `v-model` — no manual `modelValue` + `update:modelValue`
- [ ] **Vue 3.5 (§4.5)**: `onWatcherCleanup()` in every watcher that starts a timer, request, or subscription
- [ ] **Vue 3.5 (§4.5)**: `defineSlots<T>()` on every component exposing a scoped slot
