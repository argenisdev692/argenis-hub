...
- **Module finalization (MANDATORY — runs AFTER the backend suite is green):** a backend module is NOT done until this pipeline is executed, in this order, from the project root, with the real output read:
  ...
  ```
  **Then, as the LAST step, re-index the code graph** — an MCP tool call, not a
  shell command:
  ```
  index_repository(repo_path: "C:/Users/Lenovo/Herd/argenis-hub", mode: "full")
  ```
  This is MANDATORY for a new module and for any rename/move/delete. There is NO
  background auto-refresh on this project: until it runs, the new module's
  classes, handlers, ports and routes DO NOT EXIST in the graph, and every
  negative answer the next session gives ("nothing calls this", "there is no such
  class") will be confidently wrong. `mode: "full"` is required — it is what
  builds the similarity/semantic edges `semantic_query` depends on; `mode: "fast"`
  is the cheap in-flight refresh while you iterate, never the closing gate.

  Editing a method BODY without changing what it calls does not need a re-index;
  changing the SET of symbols or edges (new/renamed/deleted class, file, method,
  constructor dependency, `implements`, or route) always does.

  Verify with `index_status`: `nodes`/`edges` must have moved and `indexed_at`
  must be newer than your last source edit. Do NOT trust the `freshness` /
  `recommended_action` fields from `check_index_coverage` — on this Windows setup
  they report `metadata_changed` even for files indexed seconds ago (verified
  2026-08-25). Trust `status`, `parse_partial` ranges and `indexed_at` instead.
...
## No vendor skills — deliberate

The 8 Laravel-official skills Boost generated (`ai-sdk-development`,
`fortify-development`, `inertia-vue-development`, `infer-conventions`,
`laravel-best-practices`, `pest-testing`, `tailwindcss-development`,
`wayfinder-development`) were REMOVED on 2026-08-25. Do not reinstate them, and
do not let `boost:install` regenerate them.

Why: skill frontmatter loads into the system prompt on EVERY turn — those 8 cost
~1,565 tokens per request, permanently. They bought nothing this project did not
already have:

- **Framework docs come from Context7**, version-pinned and live. A frozen
  vendor snapshot is strictly worse (see the ABSOLUTE Context7 rule above).
- **The 6 project skills are more specific** on every overlapping topic.
- **One actively contradicted this project:** `tailwindcss-development`
  self-triggers on any mention of "tailwind" and teaches raw utilities like
  `bg-red-600`, which the CSS rules below BAN in favour of `var(--token)`.
- `infer-conventions` was already dead — it wrote through the Boost
  `record-rule` MCP tool into `.ai/rules`, both of which this project removed.

Recoverable from git history if ever needed: `git log --all -- .agents/skills`.
...
- Use the built-in tools directly — there is no filesystem MCP server in this
  project, and routing file work through one only added latency and tool-surface
  tokens on every turn:
  - `Read` to open a file · `Edit` to change an existing file · `Write` for
    brand-new files only (it refuses to overwrite a file you have not read).
  - `Grep` / `Glob` for text and path search.
- Under Claude Code's auto mode these are executed as `cat` / `sed` / `grep` /
  `find` inside Bash. That is the same operation with the same read budget — the
  ladder and the caps in **Read budget** above still apply.
- Respect the **Read budget** rules: `get_code_snippet` before a full-file read,
  and never `cat` a whole file to inspect one symbol.
...
# [MUST] Codebase exploration

- **codebase-memory-mcp is the primary structural navigation tool.** Inspect the
  existing architecture and patterns BEFORE introducing new modules, services,
  ports, repositories, controllers or events.
- Tool ladder, in order:
  1. `codebase-memory-mcp` — symbols and definitions, callers/callees, dependency
     analysis, impact analysis, module boundaries, existing implementation patterns.
  2. `Read` — only AFTER the graph has identified the relevant file/symbol.
  3. `grep` / `Glob` — exact textual searches, or when the graph cannot reliably
     answer the question.
- **Delegate the exploration, not the decision.** Graph results are large — a
  single `index_status` or a broad `search_graph` can cost 3-5k tokens, and those
  payloads are re-sent on EVERY subsequent request for the rest of the session.
  Run them inside a subagent so the payload lands in the subagent's context and
  only the conclusion returns. The three agents already exist in
  `~/.claude/agents/` — use them as-is, never inline their prompts:
  | Agent | When |
  | --- | --- |
  | `codebase-memory-scout` | Tier 1 — "where does X live", "what exists for Y". Fast, positive, provisional. The DEFAULT for any orientation question. |
  | `codebase-memory` | Tier 2 — task-directed verification before writing code that touches existing symbols. |
  | `codebase-memory-auditor` | Tier 3 — impact analysis, dead-code and any exhaustive/negative claim. |
  Delegate when the question is "what is out there" and the answer is a short
  list of files, symbols or relationships. Query the graph inline ONLY when you
  already know the exact symbol and need one `get_code_snippet`, or when the
  finalization pipeline calls `index_repository` (which no subagent profile
  exposes). Never delegate the edit itself — subagents are read-only by design.
- Never invent modules, files, classes, routes or architectural patterns. If the
  graph does not show it, verify in source before asserting that it exists.
- Coverage is best-effort, never proof of completeness: `check_index_coverage`
  every cited path, and re-verify with `grep` before any negative or exhaustive
  claim ("there is no X", "nothing calls Y").

## Dependency trees are OFF-LIMITS (enforced, verified 2026-08-27)

`vendor/`, `node_modules/` and `backup/` are NEVER the answer to a question about
this application. They are enforced in `.claude/settings.json`, not merely
advised:

- `permissions.deny` blocks `Read(**/node_modules/**)` and the build/cache trees.
  Read deny rules cover the Read and Glob tools AND the file commands Claude Code
  recognises inside Bash (`cat`, `head`, `tail`, `sed`) — so they hold in auto
  mode too, where reads are executed as `cat`.
- `permissions.ask` gates `Read(vendor/**)` — a deliberate package-source check
  costs one approval; habitual wandering stops.
- `scripts/claude-guard-hook.mjs` (PreToolUse, matcher `Bash`) closes the gap the
  permission layer leaves open: `grep`, `rg`, `find`, `ls`, `tree` and `du` are
  not "recognised file commands", so `grep -rn Foo vendor/` would otherwise run.
  The hook blocks it and returns the redirect below. Executing a binary from
  those trees (`vendor/bin/pint`, `node node_modules/vite/bin/vite.js`) stays
  allowed.

Ask the right source instead:

| Question | Correct source | NOT this |
| --- | --- | --- |
| Installed PHP package + version | `composer show --direct`, `composer show <vendor/package>` | browsing `vendor/` |
| Installed JS package + version | `package.json` | browsing `node_modules/` |
| How a package's API works | **Context7**, scoped to the installed major | reading vendor source |
| Where something lives in this app | `codebase-memory-mcp`, then `app/`, `src/`, `resources/js/`, `routes/` | `find` / `ls` sweeps |

Escape hatch: append `#allow-vendor` to the Bash command when the package source
genuinely is the answer (a suspected upstream bug, a namespace that Context7 and
the changelog disagree on). It is deliberate and auditable — never reflexive.

## Known graph blind spots (verified 2026-08-24)

- **Routes.** Laravel route definitions are runtime closures the indexer cannot
  resolve — the graph holds 2 `Route` nodes for the whole HTTP surface. For any
  route question, read the route files directly: `routes/web.php`,
  `routes/settings.php`, `routes/api.php`, `routes/console.php`, plus the module
  file mounted by `AuthServiceProvider::registerWebRoutes()` →
  `src/Modules/Auth/Infrastructure/Routes/web.php`.
- **Blade.** Templates under `resources/views/` are indexed as opaque `Module`
  nodes and 14 files are flagged `parse_partial`. A Blade view is NOT evidence of
  an implemented backend feature — confirm the controller / model / migration
  exists before treating it as one. Live example:
  `resources/views/emails/appointments/*` and
  `resources/views/exports/pdf/appointments.blade.php` have zero backing PHP.

## Read budget — the other half of the ladder

The ladder above says WHICH tool to reach for. This says HOW MUCH to pull. The
cost that blows up a thread is payload, not call count: one `cat` of a 250-line
provider costs more than six graph queries.

- **Never dump a whole file to read one symbol.** Once the graph has the
  qualified name, use `get_code_snippet` — it returns that symbol, not its 400
  neighbours. `cat`/`Read` of a full file is the fallback, not the default.
- **Intent without bodies.** `search_graph(fields: ["docstring", "signature"])`
  returns the docblock and signature of every match in one call. Most "why is
  this built this way" questions are answered there — reach for it BEFORE
  opening files, because this project documents its reasoning in docblocks.
- **Full-file reads are justified only when the file IS the answer**, and are
  capped at ~6 per exploratory task:
  - composition roots / service providers (`*ServiceProvider.php`)
  - route files (the graph cannot resolve them — see blind spots above)
  - `config/*.php`
  - files flagged `parse_partial` by `check_index_coverage`
  Anything else: snippet first, escalate to the full file only if the snippet
  proved insufficient.
- **A `for f in dir/*.php; do cat "$f"; done` loop is ONE call and N files of
  payload.** Batching calls is good; batching payload is the problem this rule
  exists to stop. Outline first (`grep -nE "class |public function"`), then pull
  only the bodies that matter.
- **Self-check before answering an "analyze / explain / how does X work"
  question:** if full-file reads outnumber graph queries by more than ~2:1, the
  ladder was skipped. Say so and prefer the graph next time.

> Under Claude Code's auto mode, `Read`/`Grep`/`Glob` are executed as `cat` /
> `grep` / `find` inside Bash. A transcript showing "Read × 0, Bash × 20" is NOT
> evidence the ladder was followed — count what the Bash calls actually did.
